...
<div class="flex gap-2 p-2 ">
    ...
    <?php
    if($_SERVER['REQUEST_URI'] !== "/freight/administrative/admin/user-management-dashboard"){
    ?>
    <a href="/freight/administrative/admin/user-management-dashboard">
    ...
    <svg class="h-8 w-8 text-indigo-500"  width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">  <path stroke="none" d="M0 0h24v24H0z"/>  <polyline points="7 7 12 12 7 17" />  <polyline points="13 7 18 12 13 17" /></svg>
        <p id="page-location" class="text-xs"></p>
    </div>
        <?php
        }else {
    ?>
    <div class='relative flex items-center'>
    <p  class="text-xs  text-pink-800">Dashboard</p>
    <svg class="h-8 w-8 text-indigo-500"  width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">  <path stroke="none" d="M0 0h24v24H0z"/>  <polyline points="7 7 12 12 7 17" />  <polyline points="13 7 18 12 13 17" /></svg>
    </div>
    <?php
        }
...
<div class="w-auto h-auto  p-1  relative">
    ...
    <a href="/freight/administrative/admin/user-management-dashboard/profile">
    <img
        class=" p-1 w-10 h-10 rounded-full ring-2 ring-indigo-300 dark:ring-indigo-500"
        src="
        <?php 
        $profile = $_SESSION['user-management-account']['profile'];
...
<script>
const page = "<?php echo $_SERVER['REQUEST_URI']; ?>".split("/freight/administrative/admin/user-management-dashboard")
const pagetText = document.querySelector("#page-location")
if(pagetText){
    pagetText.textContent = page[1].slice(1).toUpperCase()
}
