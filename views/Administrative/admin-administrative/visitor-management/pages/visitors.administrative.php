...
<script>
        ...
        const  headerArray =  [
            {title:'User',
            icon:`<svg class="h-6 w-6 text-indigo-500"  fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/>
            </svg>`
            },
            {title:'Contact',icon:`<svg class="h-6 w-6 text-indigo-500"  width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">  <path stroke="none" d="M0 0h24v24H0z"/>
            <path d="M5 4h4l2 5l-2.5 1.5a11 11 0 0 0 5 5l1.5 -2.5l5 2v4a2 2 0 0 1 -2 2a16 16 0 0 1 -15 -15a2 2 0 0 1 2 -2" />
            </svg>`
            },
            {title:'Created',icon:`<svg class="h-6 w-6 text-indigo-500"  fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
            </svg>`
            },
            {title:'Account Status',icon:`<svg class="h-8 w-8 text-indigo-500"  width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">  
    ...
    function createAppointment(id){
       window.location.href = `/freight/administrative/admin/visitor-management-dashboard/appointment-request/create/${id}`
    }
    function viewVisitorInfo(id){
       window.location.href = `/freight/administrative/admin/visitor-management-dashboard/view/${id}`
    }
    function editVisitorInfo(id){
        window.location.href = `/freight/administrative/admin/visitor-management-dashboard/edit/${id}`
    }
    function deactivateAccount(id){
        ...
        if (result.isConfirmed) {
            $.ajax({
                    type: 'post',
                    url: '/freight/deactivate-visitor-account',
                    dataType:'json',
                    data: ({visitor_id:id}),
    ...
    function displayData(res){
      ...
      res.forEach((data,idx) => {
            ...
            <tr onclick='closekAction()' class="bg-white border-b transition duration-100 ease-in-out hover:bg-gray-100 -z-[100]">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-xs  font-medium text-gray-900 ${data.deactivate !== "0" ? "text-red-500":"text-green-500"}">${data.deactivate !== "0" ? "NOT ACTIVE ACCOUNT" : "ACTIVE ACCOUNT"}</td>
                    ...
                    <div class='relative' onclick='onClickAction(${idx},this)' >
                        <svg onclick='onClickAction(${idx},this)' class="h-8 w-8 text-indigo-500 cursor-pointer "  width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">  <path stroke="none" d="M0 0h24v24H0z"/>  <circle cx="12" cy="12" r="1" />  <circle cx="12" cy="19" r="1" />  <circle cx="12" cy="5" r="1" /></svg>
                            <div id='action-container' class='hidden flex flex-col shadow-lg absolute bg-white z-[100]'>
                                <p class='cursor-pointer hover:bg-gray-200 px-4 py-2 z-[100]' onclick='viewVisitorInfo(${data.visitor_id})'>View</p>
                                <p class='cursor-pointer hover:bg-gray-200 px-4 py-2 z-[100]' onclick='createAppointment(${data.visitor_id})'>Appointment</p>
                                <p class='cursor-pointer hover:bg-gray-200 px-4 py-2 z-[100]' onclick='editVisitorInfo(${data.visitor_id})'>Edit</p>
                                <p class='cursor-pointer hover:bg-gray-200 px-4 py-2 z-[100]' onclick='deactivateAccount(${data.visitor_id})'>Deactivate</p>
                            </div>
                    </div>
                </td> 
            </tr>
...
<script>
    ...
    function sortedBy(e,arr,cb){
        ...
        }else if(e.id === 'Account Status'){
            if(arr.length > 0){
                if(arr[0].deactivate === '0'){
                    fetch("/freight/get-all-visitors-account" ,fetchDataOfVisitor(1))
                    .then(data=>data.json())
                    .then((res)=>{
                        cb(res.visitors)
                    })
                    ...
                    fetch("/freight/get-all-visitors-account" ,fetchDataOfVisitor(0))
                .then(data=>data.json())
                .then((res)=>{
                    cb(res.visitors)
                })
...
<script>
    function fetchDataOfVisitor(status){
    ...
    init(
    {url:"/freight/get-all-visitors-account",config:fetchDataOfVisitor(0)},
      {headerArray},
      root,
